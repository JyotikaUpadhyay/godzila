...
export default function LoginPage() {
  ...
  async function submit() {
    setMsg("");
    try {
      const data = await apiPost<any>(EP.authLogin, { email, password });
      auth.set(data.token);
      router.push("/orders");
    } catch (e: any) {
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-12">
      <div className="rounded-2xl border border-zinc-200 bg-white p-6">
        ...
        <h1 className="mt-2 text-2xl font-black tracking-tight">Log in</h1>
        ...
        <div className="mt-6 grid gap-3">
          ...
          <p className="text-sm text-zinc-600">
            Don’t have an account? <Link className="font-semibold hover:text-black" href="/register">Register</Link>
          </p>
        </div>
      </div>
    </main>
